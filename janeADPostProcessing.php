...
if ($SessionIsVerified == "1") {
	...
	if ($result->num_rows > 0) {
		...
		$CreateShare = $link->real_escape_string(htmlspecialchars_decode(trim($_REQUEST['CreateShare'])));
		$BaseDirectory = $link->real_escape_string(htmlspecialchars_decode(trim($_REQUEST['BaseDirectory'])));
		$ShareName = $link->real_escape_string(htmlspecialchars_decode(trim($_REQUEST['ShareName'])));
