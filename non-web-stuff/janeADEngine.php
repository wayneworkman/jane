...
if ($ActionCreate != "" && $ActionCreateText != "") {
	...
	if ($result->num_rows > 0) {
		...
		while($row = $result->fetch_assoc()) {
			...
			$COMMAND = str_replace("%Action%",$Action,$COMMAND);
			$COMMAND = str_replace("%FirstName%",$FirstName,$COMMAND);
			$COMMAND = str_replace("%LastName%",$LastName,$COMMAND);
			$COMMAND = str_replace("%MiddleInitial%",$MiddleInitial,$COMMAND);
			$COMMAND = str_replace("%MiddleName%",$MiddleName,$COMMAND);
			$COMMAND = str_replace("%Password%",$Password,$COMMAND);
			$COMMAND = str_replace("%School%",$School,$COMMAND);
			$COMMAND = str_replace("%StudentID%",$StudentID,$COMMAND);
